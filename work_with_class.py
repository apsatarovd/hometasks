...
class Car:
    ...
    def read_odometer(self):
        print(f"Пробег автомобиля: {self.odometer_reading} километров")
    
    def update_odometer(self, mileage):
        if mileage > self.odometer_reading:
            self.odometer_reading = mileage
            return f"Ваш пробег обновлен: {self.odometer_reading}"
        else:
            return "Пробег не может быть меньше текущего"
    def increment_odometer(self, miles):
        self.odometer_reading += miles
        return f"Пробег увеличен на {miles} километров"
...
print(my_car.update_odometer(7580))
print(my_car.increment_odometer(100)) 
